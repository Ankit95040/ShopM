...
export function InventoryDashboard({
  ...
  // Close menu on outside tap / Escape — mobile only
  useEffect(() => {
  ...
  // Mobile search: filter categories by category name OR any item name in that category (case-insensitive, partial)
  const searchLower = search.trim().toLowerCase();
  const filteredCategories = searchLower
    ? categories.filter(
        (cat) =>
          cat.name.toLowerCase().includes(searchLower) ||
          items.some((it) => it.categoryId === cat.id && it.name.toLowerCase().includes(searchLower))
      )
    : categories;
  ...
  return (
    <div className="space-y-6 min-w-0">
      {/* Mobile Inventory Home — reference design */}
      <div className="block md:hidden space-y-4">
        {/* Header — mobile search + menu wired, not clipped */}
        <div className="flex items-center justify-between gap-2 relative min-w-0">
          <div className="min-w-0 flex-1">
            <h1 className="text-xl font-black tracking-tight text-slate-900 truncate">Inventory</h1>
            <p className="text-xs text-slate-500 mt-0.5 truncate">Manage your stock, categories and more</p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button
              type="button"
              onClick={() => setShowMobileSearch((v) => !v)}
              aria-label={showMobileSearch ? "Close search" : "Search inventory"}
              aria-expanded={showMobileSearch}
              className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border shadow-xs transition min-h-[44px] min-w-[44px] ${showMobileSearch ? "bg-slate-900 border-slate-900 text-white" : "bg-white border-slate-200 text-slate-600 hover:bg-slate-50"}`}
            >
            ...
            <div className="relative" ref={mobileMenuRef}>
              <button
                ...
                className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border shadow-xs transition min-h-[44px] min-w-[44px] ${showMobileMenu ? "bg-slate-900 border-slate-900 text-white" : "bg-white border-slate-200 text-slate-600 hover:bg-slate-50"}`}
              >
                ...
                <div className="absolute right-0 top-full mt-2 w-52 rounded-2xl border border-slate-200 bg-white shadow-xl z-50 overflow-hidden">
                  <div className="p-2">
                    <button
                      ...
                      className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 active:bg-slate-100 transition min-h-[44px]"
                    >
                      <FolderPlus className="h-4 w-4 text-sky-600 shrink-0" />
                      Add Category
                    </button>
                    <button
                      type="button"
                      ...
                      className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 active:bg-slate-100 transition min-h-[44px]"
                    >
                      <Plus className="h-4 w-4 text-slate-900 shrink-0" />
                      Add Item
                    </button>
                    <Link
        ...
        {showMobileSearch && (
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-300 bg-white pl-9 pr-10 py-2.5 text-sm placeholder:text-slate-400 focus:border-slate-900 focus:outline-none min-h-[44px]"
            />
            {search ? (
              <button
                type="button"
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="rounded-2xl bg-sky-50 border border-sky-100 p-3 text-center">
            <div className="mx-auto flex h-8 w-8 items-center justify-center rounded-xl bg-white border border-sky-100"><Search className="h-4 w-4 text-sky-600" /></div>
            <div className="mt-2 text-lg font-black text-slate-900">{items.length}</div>
            <div className="text-[10px] font-bold text-slate-500">Total Items</div>
          </div>
          <div className="rounded-2xl bg-amber-50 border border-amber-100 p-3 text-center">
            <div className="mx-auto flex h-8 w-8 items-center justify-center rounded-xl bg-white border border-amber-100"><AlertTriangle className="h-4 w-4 text-amber-600" /></div>
            <div className="mt-2 text-lg font-black text-slate-900">{lowStockCount}</div>
            <div className="text-[10px] font-bold text-slate-500">Low Stock</div>
          </div>
          <div className="rounded-2xl bg-red-50 border border-red-100 p-3 text-center">
            <div className="mx-auto flex h-8 w-8 items-center justify-center rounded-xl bg-white border border-red-100"><AlertTriangle className="h-4 w-4 text-red-600" /></div>
            <div className="mt-2 text-lg font-black text-slate-900">{outOfStockCount}</div>
            <div className="text-[10px] font-bold text-slate-500">Out of Stock</div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-black text-slate-900">Categories</h2>
            <button onClick={() => setIsAddCategoryOpen(true)} className="inline-flex items-center gap-1 text-xs font-bold text-sky-600 hover:text-sky-700 min-h-[44px] px-2 -mr-2">
          ...
          {categories.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center">
              ...
              <h3 className="mt-4 text-sm font-bold text-slate-900">No categories yet</h3>
              <p className="text-xs text-slate-500 mt-1">Start by creating your first category to organize your inventory.</p>
              <button onClick={() => setIsAddCategoryOpen(true)} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-sky-600 px-5 py-2.5 text-sm font-bold text-white min-h-[44px]">
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center">
              ...
              <p className="text-xs text-slate-400 mt-1 break-words">No match for &quot;{search}&quot;</p>
              <button onClick={() => setSearch("")} className="mt-4 inline-flex items-center justify-center gap-2 rounded-xl bg-white border border-slate-200 px-5 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 min-h-[44px]">
                Clear search
              </button>
            </div>
          ) : (
            <div className="space-y-2">
              {filteredCategories.map((cat) => {
                ...
                return (
                  <Link key={cat.id} href={`/inventory/category/${cat.id}`} className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-xs hover:bg-slate-50 active:bg-slate-100 transition min-h-[56px]">
      ...
      {/* Desktop Inventory Home — locked */}
      <div className="hidden md:block space-y-6">
        {/* Header & Quick Action Buttons */}
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="min-w-0 flex-1">
            <h1 className="text-3xl font-black tracking-tight text-slate-900 break-words">
              {t("inventoryTitle")}
            </h1>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Link
              href="/inventory/history"
              className="flex items-center gap-1.5 rounded-2xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 shadow-xs hover:bg-slate-50 transition min-h-[44px]"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-2xl bg-slate-900 px-5 py-2.5 text-sm font-bold text-white shadow-md hover:bg-slate-800 transition active:scale-98 min-h-[44px]"
            >
        ...
        {/* Summary KPI Cards — desktop */}
        <div className="grid grid-cols-3 gap-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs min-w-0 overflow-hidden">
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50/70 p-5 shadow-xs min-w-0 overflow-hidden">
          ...
          <div className="rounded-2xl border border-red-200 bg-red-50/70 p-5 shadow-xs min-w-0 overflow-hidden">
      ...
      {/* Items — Mobile hidden on Inventory Home (details moved to category page) */}
